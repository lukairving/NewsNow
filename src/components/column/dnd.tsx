...
import { currentSourcesAtom, hiddenSourceIDsAtom } from "~/atoms"

const AnimationDuration = 200
export function Dnd() {
  const [items, setItems] = useAtom(currentSourcesAtom)
  const [hiddenSourceIDs, setHiddenSourceIDs] = useAtom(hiddenSourceIDsAtom)
  const visibleItems = items.filter(id => !hiddenSourceIDs.includes(id))
  ...
  const handleSourceError = useCallback((id: SourceID) => {
    setHiddenSourceIDs(prev => [...prev, id])
  }, [setHiddenSourceIDs])
